...
<template>
  <div class="app-container">
    <el-card class="search-card" shadow="never">
      <el-form ref="searchFormRef" :model="searchForm" :inline="true" label-width="80px">
        ...
        <el-form-item label="费用类型" prop="expense_type">
          <el-select
            v-model="searchForm.expense_type"
            placeholder="请选择类型"
            clearable
            style="width: 140px"
          >
        ...
        <el-form-item label="报销状态" prop="expense_status">
          <el-select
            v-model="searchForm.expense_status"
            placeholder="请选择状态"
            clearable
            style="width: 140px"
          >
    ...
    <el-card class="table-card" shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <div class="operations">
            <el-button
              v-permission="['module_promotion:expense:create']"
              type="primary"
              @click="handleCreate"
            >
              <i-ep-plus />
            ...
            <el-button
              v-permission="['module_promotion:expense:delete']"
              type="danger"
      ...
      >
        ...
        <el-table-column prop="expense_status" label="报销状态" width="100">
          <template #default="{ row }">
            <el-tag :type="getStatusType(row.expense_status)">
              {{ getStatusLabel(row.expense_status) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button
              v-permission="['module_promotion:expense:detail']"
              link
              type="primary"
            ...
            <el-button
              v-permission="['module_promotion:expense:update']"
              link
              type="primary"
    ...
    <el-dialog v-model="detailDialog.visible" title="费用报销详情" width="600px">
      <el-descriptions v-if="detailDialog.data" :column="2" border>
        ...
        <el-descriptions-item label="费用日期">
          {{ detailDialog.data.expense_date || "-" }}
        </el-descriptions-item>
        ...
        <el-descriptions-item label="报销状态">
          <el-tag :type="getStatusType(detailDialog.data.expense_status)">
            {{ getStatusLabel(detailDialog.data.expense_status) }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="费用描述" :span="2">
    ...
    <el-dialog
      ...
      :title="formDialog.type === 'create' ? '新增费用报销' : '编辑费用报销'"
      width="550px"
    >
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedIds.value.length} 个费用报销吗？`,
      "提示",
      {
        type: "warning",
      }
