  ...
  <div class="app-container">
    <el-card class="search-card" shadow="never">
      <el-form ref="searchFormRef" :model="searchForm" :inline="true" label-width="80px">
        <el-form-item label="表彰编号" prop="evaluation_no">
          <el-input
            v-model="searchForm.evaluation_no"
            placeholder="请输入表彰编号"
            clearable
            style="width: 150px"
          />
        ...
        <el-form-item label="表彰状态" prop="evaluation_status">
          <el-select
            v-model="searchForm.evaluation_status"
            placeholder="请选择状态"
            clearable
            style="width: 140px"
          >
    ...
    <el-card class="table-card" shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <div class="operations">
            <el-button
              v-permission="['module_promotion:evaluation:create']"
              type="primary"
              @click="handleCreate"
            >
              <i-ep-plus />
            ...
            <el-button
              v-permission="['module_promotion:evaluation:delete']"
              type="danger"
      ...
      >
        ...
        <el-table-column prop="evaluation_result" label="评选结果" width="100">
          <template #default="{ row }">
            <el-tag :type="getResultType(row.evaluation_result)">
        ...
        <el-table-column prop="evaluation_status" label="表彰状态" width="100">
          <template #default="{ row }">
            <el-tag :type="getStatusType(row.evaluation_status)">
              {{ getStatusLabel(row.evaluation_status) }}
            </el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="180" fixed="right">
          <template #default="{ row }">
            <el-button
              v-permission="['module_promotion:evaluation:detail']"
              link
              type="primary"
            ...
            <el-button
              v-permission="['module_promotion:evaluation:update']"
              link
              type="primary"
    ...
    <el-dialog v-model="detailDialog.visible" title="表彰评优详情" width="650px">
      <el-descriptions v-if="detailDialog.data" :column="2" border>
        <el-descriptions-item label="表彰编号">
          {{ detailDialog.data.evaluation_no }}
        ...
        <el-descriptions-item label="被表彰人">
          {{ detailDialog.data.personnel_name || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="表彰日期">
          {{ detailDialog.data.evaluation_date || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="评选评分">
          {{ detailDialog.data.evaluation_score || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="评选结果">
          <el-tag :type="getResultType(detailDialog.data.evaluation_result)">
            {{ detailDialog.data.evaluation_result || "-" }}
          </el-tag>
        </el-descriptions-item>
        <el-descriptions-item label="表彰状态">
          <el-tag :type="getStatusType(detailDialog.data.evaluation_status)">
            {{ getStatusLabel(detailDialog.data.evaluation_status) }}
          </el-tag>
        </el-descriptions-item>
    ...
    <el-dialog
      ...
      :title="formDialog.type === 'create' ? '新增表彰评优' : '编辑表彰评优'"
      width="600px"
    >
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="100px">
        ...
        <el-form-item label="表彰内容" prop="evaluation_content">
          <el-input
            v-model="form.evaluation_content"
            type="textarea"
            :rows="4"
            placeholder="请输入表彰内容"
          />
...
import type {
  ...
  EvaluationForm,
  EvaluationQuery,
} from "@/api/module_promotion/evaluation";
...
async function handleBatchDelete() {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要删除选中的 ${selectedIds.value.length} 个表彰评优吗？`,
      "提示",
      {
        type: "warning",
      }
