  ...
  <div class="app-container">
    ...
    <el-card class="table-card" shadow="never">
      <template #header>
        <div class="card-header">
          ...
          <div class="operations">
            <el-button
              v-permission="['module_consultation:screening:create']"
              type="primary"
              @click="handleCreate"
            >
              <i-ep-plus />
            ...
            <el-button
              v-permission="['module_consultation:screening:delete']"
              type="danger"
      ...
      >
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <el-button
              v-permission="['module_consultation:screening:detail']"
              link
              type="primary"
            ...
            <el-button
              v-permission="['module_consultation:screening:update']"
              link
              type="primary"
    ...
    <el-dialog v-model="detailDialog.visible" title="筛选条件详情" width="700px">
      <el-descriptions :column="2" border>
        <el-descriptions-item label="筛选名称" :span="2">
        ...
        <el-descriptions-item label="开始日期">
          {{ detailDialog.data?.start_date_begin || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="结束日期">
          {{ detailDialog.data?.start_date_end || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="主办方类型">
          {{ detailDialog.data?.organizer_type || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="高校数量">
          {{ detailDialog.data?.university_count_min || "-" }} -
          {{ detailDialog.data?.university_count_max || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="展位费">
          {{ detailDialog.data?.booth_fee_min || "-" }} -
          {{ detailDialog.data?.booth_fee_max || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="预计人数">
          {{ detailDialog.data?.estimated_visitors_min || "-" }} -
          {{ detailDialog.data?.estimated_visitors_max || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="合规评分">
          {{ detailDialog.data?.compliance_score_min || "-" }} -
          {{ detailDialog.data?.compliance_score_max || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="合规等级">
          {{ detailDialog.data?.compliance_level || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="信息来源">
          {{ detailDialog.data?.source_type || "-" }}
        </el-descriptions-item>
        <el-descriptions-item label="排序">
          {{ detailDialog.data?.order_by }} / {{ detailDialog.data?.order_direction }}
        </el-descriptions-item>
    ...
    <el-dialog
      ...
      :title="formDialog.type === 'create' ? '新增筛选条件' : '编辑筛选条件'"
      width="700px"
    >
      <el-form ref="formRef" :model="form" :rules="formRules" label-width="120px">
        ...
        <el-form-item label="高校数量范围">
          <el-input-number
            v-model="form.university_count_min"
            :min="0"
            placeholder="最小"
            style="width: 120px"
          />
          ...
          <el-input-number
            v-model="form.university_count_max"
            :min="0"
            placeholder="最大"
            style="width: 120px"
          />
        ...
        <el-form-item label="预计人数范围">
          <el-input-number
            v-model="form.estimated_visitors_min"
            :min="0"
            placeholder="最小"
            style="width: 120px"
          />
          <span style="margin: 0 10px">-</span>
          <el-input-number
            v-model="form.estimated_visitors_max"
            :min="0"
            placeholder="最大"
            style="width: 120px"
          />
        ...
        <el-form-item label="合规评分范围">
          <el-input-number
            v-model="form.compliance_score_min"
            :min="0"
            :max="100"
            placeholder="最小"
            style="width: 120px"
          ...
          <span style="margin: 0 10px">-</span>
          <el-input-number
            v-model="form.compliance_score_max"
            :min="0"
            :max="100"
            placeholder="最大"
            style="width: 120px"
...
const formDialog = reactive({
  ...
  data: undefined as ScreeningItem | undefined,
});
...
const handleReset = () => {
  Object.assign(searchForm, { name: undefined, is_default: undefined });
  handleSearch();
};
const handleSelectionChange = (selection: ScreeningItem[]) => {
...
const handleView = (row: ScreeningItem) => {
  ...
  await formRef.value.validate(async (valid) => {
    if (valid) {
      if (formDialog.type === "create") {
        await ScreeningAPI.create(form);
        ElMessage.success("创建成功");
      } else {
        await ScreeningAPI.update(formDialog.data!.id!, form);
...
const handleDelete = (row: ScreeningItem) => {
  ElMessageBox.confirm(`确定要删除筛选条件 "${row.name}" 吗？`, "删除确认", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "warning",
  })
    .then(async () => {
      await ScreeningAPI.delete(row.id!);
...
const handleBatchDelete = () => {
  ...
  ElMessageBox.confirm(`确定要删除选中的 ${selectedIds.value.length} 条记录吗？`, "批量删除确认", {
    ...
    .then(async () => {
      await ScreeningAPI.batchDelete(selectedIds.value);
...
const handleSetDefault = (row: ScreeningItem) => {
  ElMessageBox.confirm(`确定要将 "${row.name}" 设为默认筛选条件吗？`, "设为默认", {
    confirmButtonText: "确定",
    cancelButtonText: "取消",
    type: "info",
  })
    .then(async () => {
      await ScreeningAPI.setDefault(row.id!);
