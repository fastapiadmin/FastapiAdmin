<template>
  <el-card
    class="data-table flex-1 min-h-0 art-table-card"
  ...
  >
    ...
    <!-- 表格工具栏：art 栈迁入 ArtTableHeader #left；legacy 保持独立一行 -->
    <div
      v-if="
        !isArtTableVariant &&
    ...
    >
      ...
      >
        ...
        <div class="data-table__toolbar--right">
          <CrudToolbarActions :buttons="toolbarRightBtn" :cols="cols" :on-toolbar="handleToolbar" />
        </div>
      </slot>
    </div>

    <!-- 列表：legacy 内置 el-table；art 为 ArtTableHeader + ArtTable；#table 可完全自定义 -->
    <div class="data-table__content" :class="{ 'data-table__content--art': isArtTableVariant }">
      <template v-if="isArtTableVariant">
        <ArtTableHeader
          v-model:columns="artColumnChecks"
          :loading="loading"
          :layout="config.artToolbarLayout ?? 'refresh,size,fullscreen,columns,settings'"
          :full-class="config.artFullScreenClass ?? 'art-table-card'"
          @refresh="fetchPageData"
        >
          <!-- 仅 #left：业务按钮由插槽自建；ArtTableHeader 右侧为自带刷新/密度/全屏/列设置 -->
          <template #left>
            <div
              v-if="config.showToolbar !== false && (slots.toolbar || toolbarLeftBtn.length > 0)"
              class="crud-art-toolbar flex min-w-0 flex-1 flex-wrap items-center gap-2"
            >
              <slot
                name="toolbar"
                :toolbar-left="toolbarLeftBtn"
        ...
        <ArtTable
          ...
          :columns="artColumnsForTable"
          :data="pageData"
          :row-key="pk"
          :pagination="artPaginationBind"
          :loading="loading"
        ...
        >
          <template v-for="name in artCustomSlotNames" :key="name" #[name]="scope">
    ...
    <template v-if="showCardFooterPagination" #footer>
...
import { reactive, ref, computed, nextTick, useSlots, watch, watchEffect, shallowRef } from "vue";
import CrudToolbarLeft from "./CrudToolbarLeft.vue";
import CrudToolbarActions from "./CrudToolbarActions.vue";
import EnhancedDialog from "@/components/Core/overlays/EnhancedDialog.vue";
import ArtTable from "@/components/Core/tables/art-table/index.vue";
import ArtTableHeader from "@/components/Core/tables/art-table-header/index.vue";
import { getColumnChecks, getColumnVisibility } from "@/hooks/core/useTableColumns";
import type { ColumnOption } from "@/types/component";
import { mapCrudColsToArtColumns } from "./crudColsToArtTableColumns";
...
const legacyTableRef = ref<TableInstance>();
const artTableWrapperRef = ref<InstanceType<typeof ArtTable>>();

const isArtTableVariant = computed(() => config.value.tableVariant === "art");
const showCardFooterPagination = computed(() => showPagination && !isArtTableVariant.value);

const artColumnChecks = ref<ColumnOption[]>(getColumnChecks(mapCrudColsToArtColumns(cols.value)));

watch(
  () => config.value.tableVariant,
  (v) => {
    if (v === "art") {
      artColumnChecks.value = getColumnChecks(mapCrudColsToArtColumns(cols.value));
    }
  }
);
...
watch(
  artColumnChecks,
  () => {
    if (!isArtTableVariant.value) return;
    const ac = artColumnChecks.value;
    for (const c of cols.value) {
      const m = ac.find((x) =>
        x.type === "selection" && c.type === "selection"
          ? true
          : x.type === "index" && c.type === "index"
            ? true
            : x.prop != null && x.prop === c.prop
      );
      if (m && "show" in c) {
        const vis = getColumnVisibility(m);
        if (c.show !== vis) c.show = vis;
      }
    }
  },
  { deep: true }
...
const artColumnsForTable = computed(() =>
  artColumnChecks.value.filter((c) => getColumnVisibility(c))
);

const artPaginationBind = computed(() => {
...
const artCustomSlotNames = computed(() => {
  const names = new Set<string>();
  for (const c of artColumnsForTable.value) {
    if (c.useSlot && c.prop) {
      names.add(String(c.slotName || c.prop));
    }
  }
  return [...names];
});

const tableRef = shallowRef<TableInstance | undefined>();

watchEffect(
  () => {
    if (config.value.tableVariant === "art") {
      tableRef.value = artTableWrapperRef.value?.elTableRef as TableInstance | undefined;
    } else {
      tableRef.value = legacyTableRef.value;
    }
  },
  { flush: "post" }
);
...
:deep(.crud-embed-dialog) {
  padding-right: 0;
}

/* Art 栈：表头工具条 + 表格纵向铺满，与 examples/tables 一致 */
.data-table__content--art {
  display: flex;
  flex: 1;
