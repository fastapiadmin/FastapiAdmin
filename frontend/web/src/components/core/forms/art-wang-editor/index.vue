...
<script setup lang="ts">
  ...
  const DEFAULT_UPLOAD_CONFIG = {
    ...
    maxNumberOfFiles: 10,
    fieldName: 'file',
    allowedFileTypes: ['image/*'] as string[],
  };
  ...
  const editorConfig = computed(() => {
    ...
    // 自定义上传
    if (props.uploadConfig?.isCustomUpload && props.uploadConfig?.server) {
      config.MENU_CONF!.uploadImage!.customUpload = async (file: any, insertFn: InsertFnType) => {
        try {
