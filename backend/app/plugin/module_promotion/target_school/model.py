...
class PromotionTargetSchoolModel(ModelMixin, UserMixin):
    ...
    student_scale: Mapped[Optional[int]] = mapped_column(
        Integer,
        nullable=True,
        comment="学生规模"
    )

    intention_level: Mapped[Optional[str]] = mapped_column(
        String(20),
        nullable=True,
        comment="意向级别"
    )
    ...
    follow_person_id: Mapped[Optional[int]] = mapped_column(
        BIGINT,
        nullable=True,
        comment="跟进人ID"
    )

    last_follow_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime,
        nullable=True,
        comment="最后跟进时间"
    )
