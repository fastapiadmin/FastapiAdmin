...
async def approve_controller(
    ...
    approval_comment: str | None = Query(None, description="审批意见"),
) -> JSONResponse:
    """
    审批通过费用报销
...
async def reject_controller(
    id: Annotated[int, Path(description="费用报销ID")],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_promotion:expense:approve"]))],
    approval_comment: str = Query(..., description="审批意见"),
) -> JSONResponse:
    """
    审批拒绝费用报销
...
async def reimburse_controller(
    ...
    reimbursement_account: str | None = Query(None, description="报销账户"),
) -> JSONResponse:
    """
    报销费用
