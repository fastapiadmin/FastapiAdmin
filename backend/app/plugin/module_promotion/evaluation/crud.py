...
from .model import PromotionEvaluationModel
...
class EvaluationCRUD(CRUDBase[PromotionEvaluationModel, EvaluationCreateSchema, EvaluationUpdateSchema]):
    ...
    def __init__(self, auth: AuthSchema):
        super().__init__(model=PromotionEvaluationModel, auth=auth)

    async def get_by_id_crud(self, id: int) -> PromotionEvaluationModel | None:
        """根据ID获取详情"""
    ...
    async def create_crud(self, data: dict) -> PromotionEvaluationModel:
        """创建记录"""
        return await self.create(data=data)

    async def update_crud(self, id: int, data: dict) -> PromotionEvaluationModel:
        """更新记录"""
