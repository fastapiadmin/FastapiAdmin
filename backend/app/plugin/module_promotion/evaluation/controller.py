...
async def review_controller(
    ...
    review_comment: str | None = Query(None, description="审核意见"),
) -> JSONResponse:
    """
    审核表彰评优
...
async def approve_controller(
    ...
    approval_comment: str | None = Query(None, description="批准意见"),
    reward_type: str | None = Query(None, description="奖励类型"),
    reward_amount: float | None = Query(None, ge=0, description="奖励金额"),
) -> JSONResponse:
    """
    批准表彰评优
...
async def reject_controller(
    id: Annotated[int, Path(description="表彰评优ID")],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_promotion:evaluation:approve"]))],
    approval_comment: str = Query(..., description="拒绝意见"),
) -> JSONResponse:
    """
    拒绝表彰评优
