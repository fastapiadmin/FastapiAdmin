...
class PromotionExpenseModel(ModelMixin, UserMixin):
    ...
    expense_no: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="报销单号")
    ...
    expense_type: Mapped[str | None] = mapped_column(String(50), nullable=True, comment="费用类型")

    amount: Mapped[Numeric | None] = mapped_column(Numeric(10, 2), nullable=True, comment="金额")

    expense_date: Mapped[date | None] = mapped_column(Date, nullable=True, comment="费用日期")

    description: Mapped[str | None] = mapped_column(Text, nullable=True, comment="描述")

    attachments: Mapped[JSON | None] = mapped_column(JSON, nullable=True, comment="附件")
