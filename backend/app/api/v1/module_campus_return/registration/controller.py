...
async def list_registration_controller(
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_campus_return:registration:query"]))],
...
async def create_registration_controller(
    data: dict,
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_campus_return:registration:create"]))],
) -> ResponseSchema[dict]:
    """创建报名"""
    return ResponseSchema(data={})
...
async def update_registration_controller(
    registration_id: Annotated[int, Path(description="报名ID")],
    data: dict,
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_campus_return:registration:update"]))],
) -> ResponseSchema[dict]:
    """更新报名"""
    return ResponseSchema(data={})
...
async def delete_registration_controller(
    registration_id: Annotated[int, Path(description="报名ID")],
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_campus_return:registration:delete"]))],
) -> ResponseSchema[bool]:
    """删除报名"""
...
async def review_registration_controller(
    registration_id: Annotated[int, Path(description="报名ID")],
    data: dict,
    auth: Annotated[AuthSchema, Depends(AuthPermission(["module_campus_return:registration:review"]))],
) -> ResponseSchema[dict]:
    """审核报名"""
    return ResponseSchema(data={})
